import numpy as np
from e91lib import outcome, calc_sval, update_counts, s_uncertainty, finite_key_rate

rng = np.random.default_rng()

def three(p_flip, n, eta_degrees=45, f_ec=1.05):
    ...
    for _ in range(n):
        ...
        if ra == rb:
            ...
            i = str(a) + str(b)
            if i in store: update_counts(r, i, p_cc, p_cnc, p_ncc, counts, store)
...
def four(p_flip, n, eta_degrees=45, f_ec=1.05):
    eta = np.radians(eta_degrees)
    c_eta = np.cos(eta)
    s_eta = np.sin(eta)
...
def five(p_flip, n, eta_degrees=45, f_ec=1.05):
    eta = np.radians(eta_degrees)
    c_eta = np.cos(eta)
    s_eta = np.sin(eta)
    ...
    s_delta = (s_uncertainty(s1_counts) + s_uncertainty(s2_counts) + s_uncertainty(s3_counts)) / 3
    s_eff = s - s_delta
    qber = 1 - keylength / matchcount
    key_rate = finite_key_rate(matchcount, n, s_eff, f_ec, qber)
...
def six(p_flip, n, eta_degrees=45, f_ec=1.05):
    eta = np.radians(eta_degrees)
    c_eta = np.cos(eta)
    s_eta = np.sin(eta)
    ...
    s_delta = (s_uncertainty(s1_counts) + s_uncertainty(s2_counts) + s_uncertainty(s3_counts) + s_uncertainty(s4_counts)) / 4
    s_eff = s - s_delta
    qber = 1 - keylength / matchcount
    key_rate = finite_key_rate(matchcount, n, s_eff, f_ec, qber)
